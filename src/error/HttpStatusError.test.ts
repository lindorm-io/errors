...
describe("HttpStatusError", () => {
  ...
  describe("instanceOf", () => {
    ...
    test("should be an ExtendableError", () => {
      expect(new HttpStatusError("message", { statusCode: 100 })).toStrictEqual(expect.any(ExtendableError));
    });

    test("should be a LindormError", () => {
      expect(new HttpStatusError("message", { statusCode: 100 })).toStrictEqual(expect.any(LindormError));
    });

    test("should be a HttpStatusError", () => {
      expect(new HttpStatusError("message", { statusCode: 100 })).toStrictEqual(expect.any(HttpStatusError));
    });
  });
});
